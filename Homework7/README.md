```shell
 ...
 disbro@Dis-Bro  fraps  # Я уже активно пользуюсь Docker в Windows и Linux. Ломать ничего не хочу, много потеряю
 disbro@Dis-Bro  fraps  # Хотя у меня WSL, у Docker есть интеграция с WSL2
 disbro@Dis-Bro  fraps  # Поэтому вместо первых двух заданий, я сделаю дополнительное
 disbro@Dis-Bro  fraps  docker pull ubuntu
...
docker.io/library/ubuntu:latest
 disbro@Dis-Bro  fraps  docker run -it ubuntu
...
-rw-r--r--  1 disbro disbro     22 апр 21 17:30 test.txt.bak
 disbro@Dis-Bro  main  vi Dockerfile
 disbro@Dis-Bro  main  cat Dockerfile
FROM ubuntu:20.04
...
CMD /usr/sbin/nginx -g "daemon off;"; php-fpm -D
 disbro@Dis-Bro  ~  # Выбрал версию 20.04, так как latest при установке php-fpm
 disbro@Dis-Bro  ~  # просит интерактивный ответ при билде
 disbro@Dis-Bro  main  sudo docker build -t php-fpm .
...
Use 'docker scan' to run Snyk tests against images to find vulnerabilities and learn how to fix them
 disbro@Dis-Bro  main  docker images --filter "reference=php*"
REPOSITORY   TAG       IMAGE ID       CREATED          SIZE
php-fpm      latest    a77e7d7396a2   44 seconds ago   255MB
 disbro@Dis-Bro  main  # Не хочу показывать остальные образы, их очень много, поэтому отфильтровал по названию
